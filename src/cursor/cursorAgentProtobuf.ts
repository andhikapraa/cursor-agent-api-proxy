/**
 * Adapted from OmniRoute's open-sse/utils/cursorAgentProtobuf.ts (MIT).
 * Original protocol field tables and codec behavior are retained; this copy
 * is kept local so the proxy can use the raw Cursor AgentService transport.
 *
...
function skipGroup(buf: Buffer, offset: number): number {
  let depth = 1;
  let pos = offset;
  while (pos < buf.length && depth > 0) {
    ...
    if (wireType === 3) {
      ...
      const [length, afterLength] = decodeVarint(buf, pos);
      pos = afterLength + Number(length);
    } else if (wireType === 5) {
      pos += 4;
    } else {
      throw new Error(`unsupported grouped wireType ${wireType}`);
    }
  }
  return pos;
}
